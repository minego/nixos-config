...

{
	...
	config = {
		services.plex = rec {
			port						= 32400; # Informational only; Changing this won't do anything.
			publicURL					= "https://plex.${config.services.nginx.hostname}";
		...
		services.nginx.virtualHosts."plex.${config.services.nginx.hostname}" = {
			...
			locations."/" = {
				proxyPass				= config.services.plex.internalURL;
